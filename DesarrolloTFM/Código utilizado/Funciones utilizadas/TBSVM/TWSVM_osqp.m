%% Solving the following Quadratic Problems with the operator splitting solver for quadratic programs (OSQP)
...
function [Predict_Y,w1,w2,b1,b2,tf,W1_alp,W2_gam,alpha,gamma] = TWSVM_osqp(TestX,DataTrain,FunPara)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% TWSVM: Twin Support Vector Machine 
%
%       Predict_Y =TWSVM_osqp(TestX,DataTrain,FunPara)
% 
%       Input:
...
t0=cputime;
prob=osqp;
prob.setup(kerG1,-e2,eye(m2),Lb1,Ub1);
res= prob.solve();
alpha=res.x;
tf1=cputime-t0;
vpos=-HHG*alpha;
...
t0=cputime;
prob2=osqp;
prob2.setup(kerH1,-e1,eye(m1),Lb2,Ub2);
res= prob2.solve();
gamma=res.x;
